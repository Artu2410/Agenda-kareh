...
- [ ] `GET /api/version` returns version, commit, environment and deployedAt
...
- [ ] `GET /api/version` matches the deployed commit SHA and deploy timestamp
...
## Notes
- `GET /api/version` is intentionally public and exposes only non-sensitive release metadata: version, commit, environment and deployedAt.
