  ...
  return (
    <><div className="min-h-full w-full overflow-auto bg-slate-50 p-4 sm:p-8">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className={`mt-5 rounded-2xl border px-4 py-4 ${Number(metrics.monthly?.change) >= 0
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    </div><section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {metrics.monthly?.insuranceBreakdown?.length > 0 ? (
            metrics.monthly.insuranceBreakdown.map((item) => {
              ...
              return (
                <div key={item.name} className={`flex flex-col rounded-[1.25rem] border p-4 transition-colors lg:p-5 ${isRespiratory
                    ? 'border-purple-200 bg-purple-50 hover:border-purple-300'
                    : 'border-slate-100 bg-slate-50/50 hover:border-teal-100'}`}>
                  <span className={`truncate text-[10px] font-black uppercase tracking-[0.15em] ${isRespiratory ? 'text-purple-600' : 'text-slate-400'}`} title={item.name}>
                    {item.name}
                  </span>
                  <span className={`mt-2 text-3xl font-black ${isRespiratory ? 'text-purple-900' : 'text-slate-900'}`}>{formatCount(item.count)}</span>
      ...
      </section><section className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`rounded-xl px-4 py-2 text-xs font-black uppercase tracking-widest transition-colors ${chartType === 'bar' ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-600'}`}
            >
              Barras
            </button>
            <button
              ...
              className={`rounded-xl px-4 py-2 text-xs font-black uppercase tracking-widest transition-colors ${chartType === 'line' ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-600'}`}
            >
              Líneas
            </button>
          </div>
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div className={`rounded-2xl border px-4 py-4 ${(currentMonthRow?.volumeChange ?? 0) >= 0
      ...
      </section><section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6">
            ...
            <tbody>
              {monthlyRows.map((row, index) => (
                <tr key={row.monthKey} className={`border-b border-slate-100 ${index === 0 ? 'bg-teal-50/40' : 'bg-white'}`}>
                  ...
                  <td className={`px-4 py-4 text-right text-sm font-black ${(row.volumeChange ?? 0) >= 0 ? 'text-green-700' : 'text-rose-700'}`}
                  >
        ...
        <div className="grid gap-3 lg:hidden">
          {monthlyRows.map((row, index) => (
            <article
              key={row.monthKey}
              className={`rounded-[1.6rem] border px-4 py-4 ${index === 0
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className={`rounded-xl px-3 py-2 text-xs font-black ${(row.volumeChange ?? 0) >= 0 ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}`}
                >
                  {formatVolumeChange(row.volumeChange)}
                </div>
              </div>
