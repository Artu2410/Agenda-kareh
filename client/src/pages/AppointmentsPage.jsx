...
const AppointmentsPage = () => {
  ...
  const mobileDayRefs = useRef(new Map());
  ...
  }, [currentDate, currentWeek, mobileAgendaViewport, viewMode]);
  ...
  const mobileWeekDays = useMemo(() => (
    Array.from({ length: 7 }, (_, index) => {
      ...
      const appointmentCount = appointments.filter((appointment) => isSameDay(new Date(appointment.date), date)).length;
      const weekdayLabel = format(date, 'EEE', { locale: es }).replace('.', '');
  ...
  ), [appointments, currentDate, currentWeek]);
  ...
  const scrollMobileDayIntoView = useCallback((date) => {
    if (!mobileAgendaViewport || viewMode !== VIEW_MODE.week) return;

    const dateKey = format(date, 'yyyy-MM-dd');
    const dayElement = mobileDayRefs.current.get(dateKey);
    if (dayElement?.scrollIntoView) {
  ...
  useEffect(() => {
    if (!mobileAgendaViewport || viewMode !== VIEW_MODE.week) return undefined;

    const frameId = window.requestAnimationFrame(() => {
      scrollMobileDayIntoView(currentDate);
    });

    return () => window.cancelAnimationFrame(frameId);
  }, [currentDate, mobileAgendaViewport, scrollMobileDayIntoView, viewMode]);
  ...
  const navigatePeriod = useCallback((direction) => {
    setCurrentDate((previous) => {
      ...
      if (mobileAgendaViewport) {
        return addDays(previous, direction > 0 ? 1 : -1);
      }
  ...
  return (
    <div className="flex min-h-dvh min-w-0 flex-col overflow-hidden bg-slate-100">
      <header className="z-10 border-b bg-white px-3 pb-3 pt-4 shadow-sm sm:p-4 sm:pt-16">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center">
            <h1 className="text-lg font-black italic uppercase text-slate-800 sm:text-xl">{viewTitle}</h1>
            ...
            <div className="flex items-center justify-between rounded-lg bg-slate-100 p-1">
              <button type="button" onClick={() => navigatePeriod(-1)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 hover:bg-white" aria-label="Período anterior"><ChevronLeft size={18} /></button>
              <h2 className="px-4 text-center text-sm font-bold capitalize sm:text-[15px]">{currentRangeLabel}</h2>
              <button type="button" onClick={() => navigatePeriod(1)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 hover:bg-white" aria-label="Período siguiente"><ChevronRight size={18} /></button>
            </div>
            <button
              type="button"
            ...
            <div className="flex flex-wrap items-center rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
              <button
                ...
                className={`min-h-11 rounded-lg px-4 py-2 text-xs font-black uppercase tracking-wide transition ${
          ...
          <div className="flex w-full min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 xl:w-auto">
      ...
      <main className="flex-1 overflow-auto p-3 pt-2 sm:p-4">
        ...
        {viewMode === VIEW_MODE.week && mobileAgendaViewport ? (
          <div className="space-y-3 p-4">
              <div className="sticky top-0 z-10 -mx-4 border-b border-slate-200 bg-white/95 px-4 pb-3 pt-3 backdrop-blur">
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <button
                      type="button"
                      onClick={() => navigatePeriod(-1)}
                      className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-sm transition hover:bg-slate-50"
                      aria-label="Día anterior"
                    >
                      <ChevronLeft size={18} />
                    </button>
                    <div className="min-w-0 flex-1 text-center">
                      <p className="text-[10px] font-black uppercase tracking-[0.28em] text-slate-400">Vista diaria móvil</p>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={handleGoToToday}
                      className={`inline-flex min-h-11 shrink-0 items-center justify-center rounded-2xl border px-3 py-2 text-[10px] font-black uppercase tracking-wide transition ${
                        isToday(currentDate)
                    ...
                    <div className="calendar-scroll-container min-w-0 flex-1 overflow-x-auto">
                      <div className="flex w-max snap-x snap-mandatory items-stretch gap-2 pb-1">
                        {mobileWeekDays.map((day) => {
                          ...
                          return (
                            <button
                              ...
                              ref={(node) => {
                                if (node) {
                                  mobileDayRefs.current.set(dateKey, node);
                                } else {
                                  mobileDayRefs.current.delete(dateKey);
                                }
                              }}
                              type="button"
                              onClick={() => setCurrentDate(day.date)}
                              className={`flex min-h-11 min-w-[4.75rem] snap-center flex-col items-center justify-center rounded-2xl border px-2 py-2 text-[10px] font-black uppercase tracking-wide transition ${
                                day.isSelected
                                  ? 'border-teal-600 bg-teal-600 text-white shadow-sm'
                                  : day.isToday
                                    ? 'border-teal-200 bg-teal-50 text-teal-700 hover:bg-teal-100'
                              ...
                              aria-label={`${day.label} ${day.dayNumber} con ${day.appointmentCount} turnos`}
                            >
                              <span>{day.label}</span>
                              <span className="text-[11px] font-extrabold normal-case">{day.dayNumber}</span>
                              <span className="mt-0.5 rounded-full bg-white/80 px-1.5 py-0.5 text-[9px] font-black leading-none text-inherit">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-1 text-sm font-bold text-slate-500">
                  {mobileDayAppointments.length} turno{mobileDayAppointments.length === 1 ? '' : 's'} programado{mobileDayAppointments.length === 1 ? '' : 's'}
                </p>
              </div>
