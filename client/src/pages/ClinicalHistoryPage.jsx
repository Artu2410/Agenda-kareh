...
const ClinicalHistoryPage = () => {
  ...
  // Toggle ocultar para impresión (no-print)
  const handleToggleHideForPrint = (id) => {
    setHistoryEntries(prev => prev.map(e => e.id === id ? { ...e, hiddenForPrint: !e.hiddenForPrint } : e));
  };
  ...
  // Filtrado por rango de fechas (client-side)
  // Close datepicker when clicking outside
  useEffect(() => {
    const handler = (ev) => {
      if (showDatePicker && datePickerRef.current && !datePickerRef.current.contains(ev.target)) {
  ...
  const filteredEntries = historyEntries.filter(e => {
    ...
    // Text search filter (diagnosis or evolution)
    if (searchQuery && searchQuery.trim()) {
      const q = searchQuery.trim().toLowerCase();
      const hay = ((e.diagnosis || '') + ' ' + (e.evolution || '')).toLowerCase();
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white min-h-screen relative shadow-2xl shadow-slate-200">
      ...
      `}</style>
      {/* SCROLLBAR + LAYOUT HELPERS */}
      <style>{`
        .custom-scrollbar::-webkit-scrollbar { height: 8px; width: 8px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(16,185,129,0.35); border-radius: 9999px; }
        .custom-scrollbar { scrollbar-width: thin; scrollbar-color: rgba(16,185,129,0.35) transparent; }
        @media (max-width: 640px) {
          .history-max-h { max-height: calc(100vh - 220px); }
        }
        @media (min-width: 640px) {
          .history-max-h { max-height: 70vh; }
        }
      `}</style>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-3 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 ml-4">
                <input
                  type="search"
                  placeholder="Buscar diagnóstico / evolución..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="p-2 border rounded-xl bg-white text-sm w-64"
                />
                <div className="relative" ref={datePickerRef}>
                  <button
                    onClick={() => setShowDatePicker(!showDatePicker)}
                    className="flex items-center gap-2 px-3 py-2 border rounded-xl bg-white text-sm"
                  >
                    <Calendar size={14} />
                    {filterStart || filterEnd ? `${filterStart || '---'} → ${filterEnd || '---'}` : 'Rango fecha'}
                  </button>

                  {showDatePicker && (
                    <div className="absolute z-30 top-full mt-2 right-0 bg-white p-3 rounded-lg shadow-lg w-72 border">
                      <label className="text-[10px] font-black text-slate-400 uppercase">Desde</label>
                      <input type="date" className="w-full p-2 border rounded mb-2" value={filterStart} onChange={(e) => setFilterStart(e.target.value)} />
                      <label className="text-[10px] font-black text-slate-400 uppercase">Hasta</label>
                      <input type="date" className="w-full p-2 border rounded mb-3" value={filterEnd} onChange={(e) => setFilterEnd(e.target.value)} />
                      <div className="flex justify-between">
                        <button onClick={() => { setFilterStart(''); setFilterEnd(''); setShowDatePicker(false); }} className="text-[10px] px-3 py-1 bg-slate-50 border rounded">Limpiar</button>
                        <button onClick={() => setShowDatePicker(false)} className="text-[10px] px-3 py-1 bg-teal-600 text-white rounded">Aplicar</button>
                      </div>
                    </div>
                  )}
                </div>
                <button onClick={() => { setFilterStart(''); setFilterEnd(''); setSearchQuery(''); }} className="text-[10px] px-3 py-2 bg-slate-50 border rounded-lg">Limpiar</button>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setHistoryEntries([{
                  ...
                  diagnosis: '', evolution: '', attachments: [], status: 'typing', hiddenForPrint: false
                }, ...historyEntries])}
          ...
          <div className="space-y-8 history-max-h overflow-y-auto pr-4 custom-scrollbar">
          {filteredEntries.map((entry) => (
            <div key={entry.id} className={`border-2 border-slate-100 rounded-[2.5rem] overflow-hidden bg-white shadow-sm hover:border-teal-200 transition-all group ${entry.hiddenForPrint ? 'no-print opacity-60' : ''}`}>
              ...
              <div className="bg-slate-50 px-8 py-4 border-b border-slate-100 flex justify-between items-center group-hover:bg-teal-50/30">
                <div className="flex items-center gap-3">
                  ...
                  />
                  {entry.hiddenForPrint && <span className="text-[10px] font-bold text-rose-500 uppercase">OCULTO (no print)</span>}
                </div>
                
                {/* INDICADOR DE ESTADO VISUAL + CONTROLES */}
                <div className="flex items-center gap-2 pr-4">
                    ...
                    <button title={entry.hiddenForPrint ? 'Mostrar en impresión' : 'Ocultar en impresión'} onClick={() => handleToggleHideForPrint(entry.id)} className="p-2 rounded hover:bg-slate-100">
                      {entry.hiddenForPrint ? <EyeOff size={16} className="text-slate-600"/> : <Eye size={16} className="text-teal-600"/>}
                    </button>
                    {(!entry.id.toString().startsWith('temp-')) && (
                      <button title="Eliminar sesión" onClick={() => handleDeleteEntry(entry.id)} className="p-2 rounded hover:bg-red-50 text-red-400">
                        <Trash2 size={16} />
                      </button>
                    )}
                </div>
