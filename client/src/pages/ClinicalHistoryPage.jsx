...
const ClinicalHistoryPage = () => {
  ...
  const handleUpdatePatient = async (field, value) => {
    try {
      // Usamos patch para actualizaciones parciales y aseguramos el booleano
      await api.patch(`/patients/${patientId}`, { [field]: value });
      setPatient(prev => ({ ...prev, [field]: value }));
      toast.success("Ficha actualizada");
    } catch (err) {
      console.error(err);
      toast.error("No se pudo actualizar el antecedente");
    }
  };
  ...
  const handleFileUpload = (entryId, e) => {
    ...
    if (!file) return;
    toast.loading("Procesando...", { id: 'uploading' });
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const MAX_WIDTH = 1000; // Un poco más pequeño para asegurar fluidez
        let width = img.width;
        ...
        toast.success("Foto adjunta", { id: 'uploading' });
      };
    };
      ...
      <div className="max-w-6xl mx-auto p-6 bg-white shadow-2xl min-h-screen">
        ...
        <header className="mb-10 border-b border-slate-100 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-black text-slate-800 uppercase italic tracking-tighter">{patient?.fullName}</h2>
          </div>
          <button onClick={() => window.print()} className="no-print px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] flex items-center gap-2 shadow-lg">
          ...
          <div className="md:col-span-1 space-y-6 no-print">
            <section className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-sm">
              ...
              <div className="space-y-3 text-[11px]">
                ...
                <p className="flex justify-between border-b border-slate-200 pb-2"><b>EDAD:</b> <span className="bg-teal-100 px-2 rounded-lg font-black text-teal-700">{calculateAge(patient?.birthDate)} años</span></p>
            ...
            {filteredEntries.map((entry) => (
              <div key={entry.id} className={`session-card border-2 transition-all rounded-[2.5rem] bg-white overflow-hidden ${!entry.isVisible ? 'opacity-40 grayscale is-hidden' : 'border-slate-100 shadow-sm'}`}>
                <div className="bg-slate-50 px-8 py-4 border-b flex justify-between items-center">
                  <input 
                    type="date" 
                    className="bg-white px-3 py-1.5 rounded-xl border border-slate-200 font-black text-teal-700 text-[11px] outline-none shadow-sm" 
                    value={entry.date} 
                    onChange={(e) => {
                      setHistoryEntries(prev => prev.map(h => h.id === entry.id ? {...h, date: e.target.value} : h));
                      saveEntry({...entry, date: e.target.value});
                    }} 
                  />
                  <div className="flex items-center gap-3 no-print">
                    <button onClick={() => setHistoryEntries(prev => prev.map(h => h.id === entry.id ? {...h, isVisible: !h.isVisible} : h))} className={`p-2 rounded-xl border ${entry.isVisible ? 'bg-white text-slate-400' : 'bg-amber-100 text-amber-600 border-amber-200'}`}>
                  ...
                  {entry.attachments?.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                      {entry.attachments.map((file, idx) => (
                        <div key={idx} className="relative group aspect-square rounded-2xl overflow-hidden border border-slate-200 shadow-md bg-black">
                          <img src={file.data} alt="Adjunto" className="w-full h-full object-cover opacity-90 hover:opacity-100 transition-opacity" />
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 no-print">
                            <button 
                              onClick={() => openImage(file.data)}
                              className="bg-white p-2 rounded-full text-slate-800 shadow-xl"
                          ...
                          <button 
                            onClick={() => {
                              const updatedAtt = entry.attachments.filter((_, i) => i !== idx);
                              const updated = {...entry, attachments: updatedAtt};
