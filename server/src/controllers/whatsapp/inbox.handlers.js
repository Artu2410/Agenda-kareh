...
export const sendConversationMessage = async (req, res, prisma) => {
  ...
  try {
    ...
    if (text) {
      ...
      recordWhatsAppMessage({ direction: 'outbound', type: 'text' });
    ...
    if (file) {
      ...
      recordWhatsAppMessage({ direction: 'outbound', type: file.mimetype?.startsWith('image/') ? 'image' : 'document' });
...
export const sendWelcomeTemplate = async (req, res, prisma) => {
  ...
  try {
    ...
    recordWhatsAppMessage({ direction: 'outbound', type: result.outboundType });
